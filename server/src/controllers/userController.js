...
export async function getAllUsers(req, res) {
    try {
        const [rows] = await pool.query('SELECT id_usuario, nome, email, tipo_usuario, data_cadastro FROM usuarios');
        res.json(rows);
    } catch (err) {
...
export async function getUserById(req, res) {
    const { id } = req.params;
    try {
        const [rows] = await pool.query("SELECT id_usuario, nome, email, tipo_usuario, data_cadastro FROM usuarios WHERE id_usuario = ?", [id]);
...
export async function updateUser(req, res) {
    ...
    try {
        ...
        if (tipo_usuario) { updates.push('tipo_usuario = ?'); values.push(tipo_usuario); }
